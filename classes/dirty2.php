...
class Botasd
{
    ...
    public function checkLines(int $x, int $y, string $symbol) : bool {
        ...
        for($i = 0; $i < 2; $i++){
            if($point !== 1){
                if($point === 0) {
                    if(($this->field[$line][$point] === $this->field[$line][$point + 1])){
                        $curSymbol = $this->field[$line][$point];
                    } else if ($this->field[$line][$point] === $this->field[$line][$point + 2]){
                        $curSymbol = $this->field[$line][$point];
                    }else if ($this->field[$line][$point + 2] === $this->field[$line][$point + 1]){
                ...
                if(($this->field[$line][$point] === $this->field[$line][$point + 1])){
                    $curSymbol = $this->field[$line][$point];
                } else if ($this->field[$line][$point - 1] === $this->field[$line][$point + 1]){
                    $curSymbol = $this->field[$line][$point - 1];
                }else if ($this->field[$line][$point] === $this->field[$line][$point - 1]){
                    $curSymbol = $this->field[$line][$point];
                }
            }
            $line = $y;
        ...
        if($this->isAngular($x, $y)){
            if($x === $y){
                ...
                } else if ($this->field[1][1] === $this->field[2][2]){
                    $curSymbol = $this->field[2][2];
                }else if ($this->field[0][0] === $this->field[2][2]){
                    $curSymbol = $this->field[2][2];
                }
            } else {
                if(($this->field[2][0] === $this->field[1][1])){
                    $curSymbol = $this->field[1][1];
                } else if ($this->field[1][1] === $this->field[0][2]){
                    $curSymbol = $this->field[1][1];
                }else if ($this->field[2][0] === $this->field[0][2]){
                    $curSymbol = $this->field[2][0];
                }
            }
        }
